...
```bash
athena-spot-prices$ sam build --use-container
```
...
```bash
athena-spot-prices$ sam logs -n StockCheckerFunction --stack-name athena-spot-prices --tail
```
...
```bash
athena-spot-prices$ pip install pytest pytest-mock --user
athena-spot-prices$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name athena-spot-prices
```
